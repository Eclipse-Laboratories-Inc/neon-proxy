...
func Test_CompleteEventListValid(t *testing.T) {
	...
	assert.Equal(t, []string{"0x3078313233"}, log0["address"])
	assert.Equal(t, []string{"0x3078343536"}, log0["data"])
	assert.Equal(t, []string{"0x0"}, log0["neonEventLevel"])
	assert.Equal(t, []string{"0x1"}, log0["neonEventOrder"])
	assert.Equal(t, []string{"204"}, log0["neonEventType"])
	assert.Equal(t, []string{"0x2"}, log0["neonInnerIxIdx"])
	assert.Equal(t, []string{"true"}, log0["neonIsHidden"])
	assert.Equal(t, []string{"false"}, log0["neonIsReverted"])
	assert.Equal(t, []string{"0x4"}, log0["neonIxIdx"])
	assert.Equal(t, []string{"0x:789"}, log0["neonSolHash"])
	assert.Empty(t, log0["topics"])

	// test second log
	log1 := neonTxInfo.neonReceipt.neonTxRes.logs[1]
	assert.Equal(t, []string{"0x30783130313233"}, log1["address"])
	assert.Equal(t, []string{"0x30783130343536"}, log1["data"])
	assert.Equal(t, []string{"0x1"}, log1["neonEventLevel"])
	assert.Equal(t, []string{"0x2"}, log1["neonEventOrder"])
	assert.Equal(t, []string{"201"}, log1["neonEventType"])
	assert.Equal(t, []string{"0x5"}, log1["neonInnerIxIdx"])
	assert.Equal(t, []string{"true"}, log1["neonIsHidden"])
	assert.Equal(t, []string{"true"}, log1["neonIsReverted"])
	assert.Equal(t, []string{"0x7"}, log1["neonIxIdx"])
	assert.Equal(t, []string{"0x:10789"}, log1["neonSolHash"])
	assert.Empty(t, log0["topics"])
}
...
func Test_NeonIndexedBlockInfoAddTxHolder(t *testing.T) {
	...
	assert.Equal(t, "123:123", neonBlockInfo.neonHolders["123:123"].key.value)
}
