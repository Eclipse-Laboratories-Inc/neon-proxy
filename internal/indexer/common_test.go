...
func Test_NeonTxResultInfoAddEvent(t *testing.T) {
	...
	assert.Equal(t, []string{"0x3078303031"}, n.logs[0]["address"])
	assert.Equal(t, []string{"0x3078303032"}, n.logs[0]["data"])
	assert.Equal(t, []string{"0x0"}, n.logs[0]["neonEventLevel"])
	assert.Equal(t, []string{"0x0"}, n.logs[0]["neonEventOrder"])
	assert.Equal(t, []string{"204"}, n.logs[0]["neonEventType"])
	assert.Equal(t, []string{"0x2"}, n.logs[0]["neonInnerIxIdx"])
	assert.Equal(t, []string{"true"}, n.logs[0]["neonIsHidden"])
	assert.Equal(t, []string{"false"}, n.logs[0]["neonIsReverted"])
	assert.Equal(t, []string{"0x4"}, n.logs[0]["neonIxIdx"])
	assert.Equal(t, []string{"0x:003"}, n.logs[0]["neonSolHash"])
